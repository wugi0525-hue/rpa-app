        ...
        try {
            if (isSignUp) {
                if (password !== confirmPassword) {
                    setAuthError(language === 'ko' ? '비밀번호가 일치하지 않습니다.' : 'Passwords do not match.');
                    setIsAuthLoading(false);
                    return;
                }
            ...
            // KOR translation for common errors
            let errorMsg = error.message;
            if (error.code === 'auth/invalid-credential' || error.code === 'auth/wrong-password') {
                errorMsg = language === 'ko' ? '이메일 또는 비밀번호가 잘못되었습니다.' : 'Invalid email or password.';
            } else if (error.code === 'auth/email-already-in-use') {
                errorMsg = language === 'ko' ? '이미 사용 중인 이메일입니다.' : 'Email is already in use.';
            } else if (error.code === 'auth/weak-password') {
                errorMsg = language === 'ko' ? '비밀번호는 6자리 이상이어야 합니다.' : 'Password should be at least 6 characters.';
            }
